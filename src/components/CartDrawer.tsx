      ...
      <div id="cart-drawer-panel" className="relative w-full max-w-md h-full bg-white border-l border-pink-100 shadow-2xl flex flex-col animate-slide-left text-slate-800">
        ...
        <div className="p-5 border-b border-pink-100 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <ShoppingBag className="w-5.5 h-5.5 text-pink-500" />
            <h2 className="text-lg font-bold font-serif tracking-tight text-slate-800">Your Gourmet Basket</h2>
            <span className="bg-pink-50 text-pink-600 font-mono text-xs px-2.5 py-0.5 rounded-full font-bold">
          ...
          <button 
            id="close-cart-btn"
            onClick={onClose} 
            className="p-1.5 rounded-lg bg-pink-105 bg-pink-100 hover:bg-pink-200 text-pink-600 hover:text-pink-800 transition-all cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {cart.length > 0 && (
            <div className="bg-pink-50/40 border border-pink-100/60 rounded-2xl p-4 space-y-3.5 mb-2 text-left">
              <h4 className="text-[11px] font-sans font-black uppercase text-pink-600 tracking-wider flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5 fill-current" /> Yummy Campaign Milestones
              </h4>
              ...
              <div className="space-y-1">
                <div className="flex justify-between items-center text-xs">
                  ...
                  <span className="font-mono text-[10px] font-bold text-pink-700">
              ...
              <div className="space-y-1 pt-2 border-t border-pink-100/40">
                <div className="flex justify-between items-center text-xs">
          ...
          {cart.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
              <div className="w-16 h-16 rounded-full bg-pink-50 flex items-center justify-center border border-pink-100">
                <ShoppingBag className="w-7 h-7 text-pink-300" />
              </div>
              <div>
                ...
                <p className="text-xs text-slate-450 max-w-xs mt-1 leading-relaxed">
                  Add some of our delicious traditional Lebanese baklava or gourmet candy imports.
                </p>
              </div>
              <button
                onClick={onClose}
                className="px-5 py-2.5 text-xs font-semibold bg-pink-500 hover:bg-pink-600 text-white rounded-lg cursor-pointer transition-all shadow-sm"
              >
                Explore Items
              </button>
            </div>
            ...
            cart.map((item) => {
              ...
              return (
                <div 
                  ...
                  className="flex items-center gap-4 p-3 bg-pink-50/20 border border-pink-100/60 rounded-xl"
                >
                  <img
                    src={item.product.image_url}
                    alt={item.product.name}
                    className="w-16 h-16 object-cover rounded-lg border border-pink-100 bg-pink-50/10"
                  ...
                  <div className="flex-1 min-w-0 text-left">
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      <span className="text-sm font-bold text-pink-600 font-mono">${(itemPrice * item.quantity).toFixed(2)}</span>
                      {hasDiscount && (
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <div className="flex items-center bg-pink-100/60 border border-pink-200 rounded-lg p-0.5">
                      <button
                        onClick={() => onUpdateQuantity(item.product.id, item.quantity - 1)}
                        className="p-1 text-pink-600 hover:text-pink-800 hover:bg-pink-200/50 rounded cursor-pointer"
                      >
        ...
        {cart.length > 0 && (
          <div className="p-5 border-t border-pink-100 bg-white space-y-4">
            ...
            <div className="space-y-2 text-sm text-slate-600 font-sans">
              ...
              {totalSaved > 0 && (
                <div className="flex justify-between text-pink-600 text-xs font-semibold">
                  <span className="flex items-center gap-1">
                    <Tag className="w-3.5 h-3.5" /> Special Flash Discounts
                  </span>
              ...
              <div className="flex justify-between items-center text-xs text-slate-400">
                <span className="flex items-center gap-1">
                  <Truck className="w-3.5 h-3.5 text-pink-500" /> Lebanon Delivery Fee
                </span>
                {deliveryFee === 0 ? (
                  <span className="font-mono text-pink-600 font-bold uppercase text-[10px] tracking-wide">FREE</span>
                ) : (
              ...
              {totalDiscounted < deliveryThreshold && (
                <p className="text-[11px] text-pink-550 text-left font-light bg-pink-50/50 p-2 rounded-lg border border-pink-100/50">
              ...
              {totalDiscounted >= giftThreshold && (
                <div className="bg-pink-50 p-2.5 rounded-lg border border-pink-100/80 flex items-center gap-2 text-left animate-pulse">
                  <span className="text-lg">🎁</span>
                  <div>
                    <p className="text-xs font-extrabold text-pink-700">Free Premium Gift Earned!</p>
                    <p className="text-[10px] text-slate-500 font-light">A complimentary mystery wellness booster has been added to your parcel.</p>
                  </div>
                </div>
              )}

              <div className="border-t border-pink-100 my-2 pt-2 flex justify-between text-base font-bold text-slate-900">
                <span>Total (Cash on Delivery)</span>
                <span className="font-mono text-pink-600 text-lg">${grandTotal.toFixed(2)}</span>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-3.5 bg-gradient-to-r from-pink-500 to-rose-600 hover:from-pink-400 hover:to-rose-500 text-white font-bold rounded-xl transition-all shadow-md active:scale-98 cursor-pointer"
            >
